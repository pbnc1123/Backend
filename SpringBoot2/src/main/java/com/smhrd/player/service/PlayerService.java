...
public class PlayerService {
	...
	public JSONArray PlayerList() {
		...
		// 리액트 서버에 이미지를 직접 넘겨야 하기 때문에 이미지 데이터 가공 해야함
		// imgSrc파일이름 -> 실제파일 -> 이미지(byte형태의 문자열)로 응답
		...
		for(Player p : p_list ) {
			...
			// System.out.println("fileString: " + fileString); 동작확인 후 주석처리
			
			// 2. imgSrc필드 : 원래는 파일 이름 -> 변환된 byte 문자열
